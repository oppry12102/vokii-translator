    ...
    public static String buildSystemPrompt(SessionContext ctx, ToolRegistry registry) {
        ...
        if (registry != null && !registry.names().isEmpty()) {
            sb.append("\n\nYou are EAVESDROPPING on a real conversation between two ")
              ...
              .append("'重新翻译上一句', '温度调到0.7', '你能做什么', ")
              .append("'记住张三叫 Zhang San', '改一下张三是 Lao Zhang', '字体变大'. The available ")
              ...
              .append("tool: '切换到普通模式' means toggle_cascade), listing ")
              .append("commands, remembering how to translate a specific term or ")
              .append("name (remember_term — also use it when the user explicitly ")
              .append("corrects a translation; pass the term and its translation), ")
              .append("viewing the stored terms (list_terms), and changing the ")
              .append("transcript font size (set_font_size — direction ")
              .append("'larger'/'smaller').\n")
